...
2. Invoke the **hybrid-dev** skill (project-local) with:
   - repo: this repo's absolute path · branch: `autodev/m<n>-<slug>` (hybrid-dev creates it — do NOT pre-create)
...
5. status != done → **RETRY path** (below).
...
If M<n> needs a structural `autodev/gate.sh` extension (new step kind — committed acceptance tests usually suffice via the existing test step): edit gate.sh NOW, commit, re-run the gate. The S5 close-out is docs-only; a gate delta merging unreviewed is forbidden.
